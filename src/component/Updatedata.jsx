...
function UpdateData() {
    ...
    const [formData, setFormData] = useState((datalocation != {} && datalocation != null && datalocation != undefined) ? datalocation : null);
    useEffect(() => {
        if (sessionStorage.getItem('role') != 'admin') {
            navigate("/Dashboard");
        }
        if (datalocation != {} && datalocation != null && datalocation != undefined) {
            // setFormData(datalocation);
        } else {
            navigate("/manage");
        }
    ...
    const handleSubmit = (event) => {
        ...
        // sessionStorage.setItem('statusInsertOrUpdateData','update');
        const requestData = {
            type: "updatemain",//sessionStorage.getItem('statusInsertOrUpdateData')
            data: formData,
    ...
    return (
        <>
            <Header />
            <LinkPage />
            {(datalocation != {} && datalocation != null && datalocation != undefined) ? (
                <div className="flex flex-row justify-center items-center p-10">
                    <form onSubmit={handleSubmit} method="post" className="flex flex-col justify-between p-10 max-w-screen-lg w-full items-center border rounded-2xl border-yellow-500">
                        <b className="w-full h-12 flex justify-start  items-center  border-b mb-5" >ปีงบประมาณ :</b>
                        <input readOnly type="number" name="fiscalyear" value={formData.fiscalyear} onKeyDown={prevententersend} onChange={handleChange} className="w-full h-12 rounded-xl border-gray-600 " min={0} required />
                        ...
                        <button type="submit" className="w-full h-12 rounded-xl border mt-5 bg-yellow-500 text-white  transition-colors duration-300 hover:bg-white hover:text-black">อัพเดต</button>
